...
std_msgs__msg__Bool brake_input_msg;
...
bool brakeOverride = false; // global flag

void brake_callback(const void * msgin) {
    const std_msgs__msg__Bool * msg = (const std_msgs__msg__Bool *)msgin;
    brakeOverride = msg->data;
}
...
void setup() {
  ...
  rclc_executor_add_subscription(&executor, &brake_subscriber, &brake_input_msg, &brake_callback, ON_NEW_DATA);
...
void loop() {
  ...
  if (updated && myController && myController->isConnected() && myController->hasData()) {
    ...
    if (brake < 1 && throttle > 5) {
      if (myController->x() || brakeOverride) {
        drive = 1000;
        mode = 0;
      } else {
    ...
    pwmSteerUs = map(steerX, -511, 512, 1000, 2000);  // Left joystick X (-511 to 512)
